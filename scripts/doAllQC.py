...
if __name__ == '__main__':
    ...
    for s in SeqIO.parse('BOLD_Chordata_Australian_families_only.fasta', 'fasta'):
        all_seqs.append(s)
    ...
    with open('BOLD_Chordata_Australian_families_only.taxids.txt') as fh:
        for line in fh:
            ll = line.rstrip().split('\t')
            # ['>GBGC12631-13', 'Trachylepis gravenhorstii', '185474', 'cellular organisms;Eukaryota;Opisthokonta;Metazoa;Eumetazoa;Bilateria;Deuterostomia;Chordata;Craniata;Vertebrata;Gnathostomata;Teleostomi;Euteleostomi;Sarcopterygii;Dipnotetrapodomorpha;Tetrapoda;Amniota;Sauropsida;Sauria;Lepidosauria;Squamata;Bifurcata;Unidentata;Scinciformata;Scincidae;Mabuyinae;Trachylepis;Trachylepis gravenhorstii', 'Eukaryota;Chordata;Lepidosauria;Squamata;Scincidae;Trachylepis;Trachylepis gravenhorstii']
            try:
                name, species, taxid, long, short = ll
                name = name.replace('>','')
    ...
    with open('1-selfblast/selfblastdb.fasta', 'w') as fastaout\
        ...
        for counter, s in enumerate(all_seqs):
            # naming scheme:
            ...
            if 'cf.' in s.description or 'sp.' in s.description or ' x ' in s.description or ' X ' in s.description \
                ...
                if len(all_species_for_genus_clean) >= 1: 
                    ...
                    removed[thisid] =  f'vague species identification ({s.description}), {len(all_species_for_genus_clean)-1} other species exist'
                    continue
            ...
            new_name = f'{counter}_{thisid} {taxid} {species} {s.description}'
            fastaout.write(f'>{new_name}\n{str(s.seq)}\n')
            taxout.write(f'{counter}_{thisid} {taxid}\n')

    # make the blast database
    print(os.popen('makeblastdb -dbtype nucl -in 1-selfblast/selfblastdb.fasta -parse_seqids -taxid_map 1-selfblast/selfblastdb_taxIDs.txt').read())
    ...
    if not os.path.exists('1-selfblast/selfblastdb.results.tsv'):
        os.popen('blastn -db 1-selfblast/selfblastdb.fasta -query 1-selfblast/selfblastdb.fasta -out 1-selfblast/selfblastdb.results.tsv -num_threads 400 -outfmt "6 qseqid sseqid staxids sscinames scomnames sskingdoms pident length qlen slen mismatch gapopen gaps qstart qend sstart send stitle evalue bitscore qcovs qcovhsp" -perc_identity 100 -qcov_hsp_perc 98').read()
